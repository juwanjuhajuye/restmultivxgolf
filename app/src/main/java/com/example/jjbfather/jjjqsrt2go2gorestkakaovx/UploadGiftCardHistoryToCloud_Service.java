...
public class UploadGiftCardHistoryToCloud_Service extends Service implements Runnable {
    Context context;
    // DB 객체 선언
    static DatabaseInit dbInitForUploadCloud;
    Intent mIntent;

    public UploadGiftCardHistoryToCloud_Service() {
        this.context = this;
        // DatabaseInit 객체 생성
    ...
    public void run() {
        try {
            ...
            UploadGiftCardHistoryToCloud uploadInstance = new UploadGiftCardHistoryToCloud();
            uploadInstance.sendGiftCardHistoryDataToCloud(true);
        } catch (Exception e) {
