...
public class API_download_giftcardbalance extends AsyncTask {
    String mReturnValue = "";
    ...
    public API_download_giftcardbalance(String paramGcNumber) {
        if (!GlobalMemberValues.isStrEmpty(paramGcNumber)) {
            // 01022024
            // gift card type 에 필요한 작업
            // sidx 추가
            mStrUrl = GlobalMemberValues.API_WEB + "API_DownloadGiftCardBalance_ForAndroid.asp?scode=" + GlobalMemberValues.SALON_CODE +
                    "&sidx=" + GlobalMemberValues.STORE_INDEX +
                    "&gcnumber=" + GlobalMemberValues.getEncoderUtf8(GlobalMemberValues.getReplaceText(paramGcNumber, " ", "|||"));
