...
public class API_deviceuseable_check extends AsyncTask {
    String mReturnValue = "";
    ...
    public API_deviceuseable_check(String paramSerialnumber) {
        if (!GlobalMemberValues.isStrEmpty(paramSerialnumber)) {
            mStrUrl = GlobalMemberValues.API_WEB + "API_CheckAppUseYN_ForAndroid.asp?" +
                    "sidx=" + GlobalMemberValues.STORE_INDEX +
                    "&serialnumber=" +
                    GlobalMemberValues.getEncoderUtf8(GlobalMemberValues.getReplaceText(paramSerialnumber.toUpperCase(), " ", "|||"));
            GlobalMemberValues.logWrite("AppCheckApiMStrUrl", "url : " + mStrUrl + "\n");
        }
    }
