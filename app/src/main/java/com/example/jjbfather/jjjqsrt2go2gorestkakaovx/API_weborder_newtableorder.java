...
public class API_weborder_newtableorder extends AsyncTask {
    ...
    public API_weborder_newtableorder(String paramStr) {
        if (!GlobalMemberValues.isStrEmpty(paramStr)) {
            // 07092024
            // 예약내역 가져오기 관련
            if (!GlobalMemberValues.isStrEmpty(BayReservationViewer.mHoldcode)) {
                mStrUrl = GlobalMemberValues.API_WEB + "API_Check_GetReservation_ForAndroid.asp?sidx=" + paramStr + "&stcode=" + GlobalMemberValues.STATION_CODE + "&holdcode=" + BayReservationViewer.mHoldcode;
                BayReservationViewer.mHoldcode = "";
            } else {
