...
public class PosbankPrinter4 {
    ...
    public PosbankPrinter4(Context context, String paramLoc){
        ...
        switch (mLocation) {
            ...
            case "SETTINGS" : {
                dialContext = SettingsDeviceKitchenPrinter3.context;
                break;
            }
            default: {
                if (mContext == null) {
    ...
    public void setConnect(boolean paramDialog){
        if (GlobalMemberValues.PRINTERCONNECTEDSTATUS4 == false) {
            mPrinter = new Printer(mContext, mHandler, null);
            asyncDialog = new ProgressDialog(mContext);
            asyncDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            asyncDialog.setMessage("Loading to connect printer device (POSBANK A11 - kitchen3)...");
            asyncDialog.setCanceledOnTouchOutside(false);
            asyncDialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
                @Override
                public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
            ...
            switch (msg.what) {
                case Printer.MESSAGE_STATE_CHANGE:
                    switch (msg.arg1) {
                        case Printer.STATE_CONNECTED:
                            ...
                            GlobalMemberValues.PRINTERCONNECTEDSTATUS4 = true;
                            Toast.makeText(mContext, "PosBank Printer Connect Completed - kitchen3 !", Toast.LENGTH_LONG).show();
                            GlobalMemberValues.displayDialog(dialContext, "Printer Connected Message", "PosBank A11 Printer Connect Completed - kitchen3 !", "Close");
                ...
                case Printer.MESSAGE_DEVICE_NAME:
                    ...
                    if (!GlobalMemberValues.isStrEmpty(mConnectedDeviceName)) {
                        ...
                        if (GlobalMemberValues.checkConnectedPosBankKitchPrinterByIp("4",
                        ...
                        String strQuery = " update salon_storestationsettings_deviceprinter4 set " +
                                " printername = '" + SettingsDeviceKitchenPrinter3.selectedItemPrinterNameSpinner + "', " +
                        ...
                        if (returnResult == "N" || returnResult == "") {
                            ...
                            GlobalMemberValues.NETWORKPRINTERIP4 = tempConnectedDeviceName;

                            SettingsDeviceKitchenPrinter3.networkIpTextView1.setText(ipAddrs[0]);
                            SettingsDeviceKitchenPrinter3.networkIpTextView2.setText(ipAddrs[1]);
                            SettingsDeviceKitchenPrinter3.networkIpTextView3.setText(ipAddrs[2]);
                            SettingsDeviceKitchenPrinter3.networkIpTextView4.setText(ipAddrs[3]);

                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName2 : " + ipAddrs[0] + "." + ipAddrs[1] + "." + ipAddrs[2] + "." + ipAddrs[3] + "\n");
                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName4 : " + GlobalMemberValues.NETWORKPRINTERIP4 + "\n");
