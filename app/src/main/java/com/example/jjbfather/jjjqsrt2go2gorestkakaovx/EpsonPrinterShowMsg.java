...
public class EpsonPrinterShowMsg {
...
//        show(msg, context);
    ...
    public static void showExceptionConnectTest(Exception e, String method, Context context, String printerNum) {
        String msg = "";
        if (e instanceof Epos2Exception) {
            msg = String.format(
                    "%s\n\t%s\n%s\n\t%s",
                    printerNum + " " + context.getString(R.string.title_err_code),
                    getEposExceptionText(((Epos2Exception) e).getErrorStatus()),
                    context.getString(R.string.title_err_method),
                    method);
        }
