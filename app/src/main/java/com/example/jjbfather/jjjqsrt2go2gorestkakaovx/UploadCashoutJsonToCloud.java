...
public class UploadCashoutJsonToCloud extends Service implements Runnable {
    Context context;
    // DB 객체 선언
    ...
    String TAG = "cashoutjsonuploadlog";


    public UploadCashoutJsonToCloud() {
        this.context = this;
        // DatabaseInit 객체 생성
    ...
    public void sendCashoutJsonToCloud() {
        if (GlobalMemberValues.GLOBALNETWORKSTATUS > 0) {
            ...
            String strQueryString = " select idx, jsonstr, cashoutNum, endofdayNum, employeeidx, employeename from salon_sales_cashout_json " +
            ...
            while (cartCursor.moveToNext()) {
                ...
                cashoutNum = GlobalMemberValues.getDBTextAfterChecked(cartCursor.getString(2), 1);
                endofdayNum = GlobalMemberValues.getDBTextAfterChecked(cartCursor.getString(3), 1);
                employeeidx = GlobalMemberValues.getDBTextAfterChecked(cartCursor.getString(4), 1);
                employeename = GlobalMemberValues.getDBTextAfterChecked(cartCursor.getString(5), 1);

                GlobalMemberValues.logWrite(TAG + "WANHAYEJJJ", tempi + "-jsonstr : " + jsonstr + "\n");
//                JSONObject tempJson = null;
                if (!GlobalMemberValues.isStrEmpty(jsonstr)) {
//                        tempJson = new JSONObject(jsonstr);
//                        tempJson.put("scode", GlobalMemberValues.SALON_CODE);
//                        tempJson.put("sidx", GlobalMemberValues.STORE_INDEX);
//                        tempJson.put("stcode", GlobalMemberValues.STATION_CODE);
//                        tempJson.put("cashoutnum", cashoutNum);
//                        tempJson.put("endofdaynum", endofdayNum);
//                        tempJson.put("employeeidx", employeeidx);
//                        tempJson.put("employeename", employeename);

                    jsonstr = jsonstr + "-WJJJ-" + cashoutNum +
                            "-WJJJ-" + endofdayNum +
                            "-WJJJ-" + employeeidx +
                            "-WJJJ-" + employeename +
                ...
                strUpdateQuery = "update salon_sales_cashout_json set isCloudUpload = 1 " +
                        " where idx = '" + posidx + "' ";
        ...
        // 서비스 중지
        if (GlobalMemberValues.CURRENTACTIVITYOPENEDSERVICE_CASHOUTJSON != null && GlobalMemberValues.CURRENTSERVICEINTENT_CASHOUTJSON != null)
            GlobalMemberValues.CURRENTACTIVITYOPENEDSERVICE_CASHOUTJSON.stopService(GlobalMemberValues.CURRENTSERVICEINTENT_CASHOUTJSON);
    }
    ...
    public void sendJsonDataToCloud(String JsonMsg, String paramUpdateSql) {
        ...
        jsonSentResult = 99;
        jsonSentResultStr = "";
        String tempApiURL = "API_DOWNLOAD_CashoutJson.asp";
