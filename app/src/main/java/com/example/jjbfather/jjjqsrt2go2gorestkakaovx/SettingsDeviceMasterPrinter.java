...
public class SettingsDeviceMasterPrinter extends Activity {
    ...
    private Switch logoprintingonreceiptynSwitch, master_use_yn_Switch;
    ...
    public RadioGroup MasterSelectPrintRadioGroup;
    public RadioButton ReceiptRadioButton, KitchenRadioButton, LabelRadioButton;
    ...
    public static String selectedItemPrinterNameSpinner = GlobalMemberValues.MASTER_PRINTERGROUP[0];
    ...
    private int m_i_receiptKind = 0;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        requestWindowFeature(Window.FEATURE_NO_TITLE);      // 타이틀바 안보이게
        setContentView(R.layout.activity_settings_device_master_printer);
    ...
    public void setContents() {
        ...
        String strQuery = "select printername, autoreceipt, receiptfontsize, receiptpapercount, receiptfooter, " +
                ...
                " taxexemptprintingyn, logoprintingonreceiptyn, logoimgpath, printreceiptkind, masteruseyn " +
                " from salon_storestationsettings_deviceprinter_master ";
        ...
        if (settingsDevicePrinterCursor.getCount() > 0 && settingsDevicePrinterCursor.moveToFirst()) {
            ...
            String tempPrintreceiptkind = GlobalMemberValues.getDBTextAfterChecked(settingsDevicePrinterCursor.getString(34), 1);
            String tempMasterUseYN = GlobalMemberValues.getDBTextAfterChecked(settingsDevicePrinterCursor.getString(35), 1);
            ...
            if (!GlobalMemberValues.isStrEmpty(tempMasterUseYN)){
                vMasterUse = GlobalMemberValues.getIntAtString(tempMasterUseYN);
            }
            ...
            if (GlobalMemberValues.isStrEmpty(tempPrintreceiptkind)){
                i_receipt_kind = 0;
            } else {
                i_receipt_kind = GlobalMemberValues.getIntAtString(tempPrintreceiptkind);
            }
        }


        // Forward / Reverse -------------------------------------------------------------------------------------------------
        MasterSelectPrintRadioGroup = (RadioGroup)findViewById(R.id.MasterSelectPrintRadioGroup);
        MasterSelectPrintRadioGroup.setOnCheckedChangeListener(checkRadioGroup);

        ReceiptRadioButton = (RadioButton)findViewById(R.id.ReceiptRadioButton);
        ReceiptRadioButton.setTextSize(ReceiptRadioButton.getTextSize() * GlobalMemberValues.getGlobalFontSize());

        KitchenRadioButton = (RadioButton)findViewById(R.id.KitchenRadioButton);
        KitchenRadioButton.setTextSize(KitchenRadioButton.getTextSize() * GlobalMemberValues.getGlobalFontSize());

        LabelRadioButton = (RadioButton)findViewById(R.id.LabelRadioButton);
        LabelRadioButton.setTextSize(LabelRadioButton.getTextSize() * GlobalMemberValues.getGlobalFontSize());

        switch (i_receipt_kind) {
            case 0 : {
                ReceiptRadioButton.setChecked(true);
                break;
            }
            case 1 : {
                KitchenRadioButton.setChecked(true);
                break;
            }
            case 2 : {
        ...
        for (int i = 0; i < GlobalMemberValues.MASTER_PRINTERGROUP.length; i++) {
            mGeneralArrayListForSpinnerPrinterName.add(GlobalMemberValues.MASTER_PRINTERGROUP[i]);
        }
        ...
        for (int i = 0; i < GlobalMemberValues.MASTER_PRINTERGROUP.length; i++) {
            if (vPrintername.equals(GlobalMemberValues.MASTER_PRINTERGROUP[i])) {
                tempPNPosition = i;
            }
        }
        ...
        master_use_yn_Switch = (Switch)findViewById(R.id.master_use_yn_Switch);
        if (vMasterUse == 0){
            master_use_yn_Switch.setChecked(false);
        } else {
            master_use_yn_Switch.setChecked(true);
        }
...
//                    }
                ...
                case R.id.testPrintButton : {
                    ...
                    GlobalMemberValues.printMaster(null, context, "testprint","");
                    break;
                }
            }
        }
                    ...
                    if (resultValue > 0) {
                        ...
                        switch (mSavedPrinterName) {
                            ...
                            case "Giant-100" : {
                                ...
                                GlobalMemberValues.NETWORKPRINTERIP_MASTER = tempip;
                                if (!networkPrinterPortTextView.getText().toString().isEmpty()) {
                                    GlobalMemberValues.NETWORKPRINTERPORT = networkPrinterPortTextView.getText().toString();
                                }
                                GlobalMemberValues.printReceiptByJHP(null, context, "testprint");
                                break;
                            }
    ...
    public int setSettingsDevicePrinter() {
        ...
        int i_insPrintReceiptKind = 0;

        int i_insMasterUseYN = 0;
        ...
        if (master_use_yn_Switch.isChecked()){
            i_insMasterUseYN = 1;
        } else {
            i_insMasterUseYN = 0;
        }
        ...
        i_insPrintReceiptKind = m_i_receiptKind;
        ...
        String qurey = "SELECT * FROM salon_storestationsettings_deviceprinter_master WHERE idx";
        String qurey_result = dbInit.dbExecuteReadReturnString(qurey);
        if (qurey_result.isEmpty()){
            qurey = "insert into salon_storestationsettings_deviceprinter_master(printername)values('NO PRINTER')";
        ...
        String updStrQuery = "update salon_storestationsettings_deviceprinter_master set " +
...
//                " taxexemptprintingyn = '" + insTaxexemptprintingyn + "', " +
                ...
                " printreceiptkind = '" + i_insPrintReceiptKind + "', " +
                " masteruseyn = '" + i_insMasterUseYN + "', " +
...
//                " menulistprintingyn = '" + insMenulistprintingyn + "', " +
        ...
        for (String tempQuery : strUpdateQueryVec) {
            GlobalMemberValues.logWrite("SettingsDevicePrinterMaster", "query : " + tempQuery + "\n");
        }
    ...
    RadioGroup.OnCheckedChangeListener checkRadioGroup = new RadioGroup.OnCheckedChangeListener() {
        ...
        public void onCheckedChanged(RadioGroup group, int checkedId) {
            switch (group.getId()) {
                case R.id.MasterSelectPrintRadioGroup : {
                    switch (checkedId){
                        case R.id.ReceiptRadioButton :
                            m_i_receiptKind = 0;
                            break;
                        case R.id.KitchenRadioButton:
                            m_i_receiptKind = 1;
                            break;
                        case R.id.LabelRadioButton:
                            m_i_receiptKind = 2;
                            break;
                    }
                    break;
