 ...
 */
public class APIDownLoad_coupon_imgtype extends AsyncTask {

    /** 데이터베이스 테이블 필드값 선언 및 초기화 ********************/
    /** 테이블에 따라 여기를 수정해야 함 ********************/
    String mDbTableName = "coupon_imgtype";
    ...
    protected Object doInBackground(Object[] params) {
        try {
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (isReturnCode && tagName.equals("ReturnCode")) {
                        ...
                        if (mApiReturnCode.equals("00")) {
                            isPossible = true;
                            GlobalMemberValues.logWrite("coupon_imgtype", "성공" + "\n");
                        } else {
                            isPossible = false;
                            GlobalMemberValues.logWrite("coupon_imgtype", "에러" + "\n");
                        }
                        GlobalMemberValues.logWrite("coupon_imgtype", "ReturnCode : " + mApiReturnCode + "\n");
                    }

                    if (isPossible && isDataTag) {
                        //GlobalMemberValues.logWrite("coupon_imgtype", "성공일때만.." + "\n");
                        if (tagName.equals("idx")) {
                            idx = xpp.getText();
                        }
                        if (tagName.equals("cidx")) {
                            cidx = xpp.getText();
                        }
                        if (tagName.equals("imgonly")) {
                            imgonly = xpp.getText();
                        }
                    }
                    ...
                    if (tagName.equals("Data")) {
                        ...
                        mInsertSqlQuery = "insert into " + mDbTableName +
                                ...
                                "'" + GlobalMemberValues.getDBTextAfterChecked(cidx, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(imgonly, 0) + "' " +
                                ")";
                        sqlQueryVecIns.add(mInsertSqlQuery);

                        // 수정쿼리 조합하기
                        mUpdateSqlQuery = "update " + mDbTableName + " set " +
                                " cidx = '" + GlobalMemberValues.getDBTextAfterChecked(cidx, 0) + "', " +
                                " imgonly = '" + GlobalMemberValues.getDBTextAfterChecked(imgonly, 0) + "' " +
                                " where idx = " + idx;
                        ...
                        GlobalMemberValues.logWrite("coupon_imgtype", "mQueryCollection : " + mQueryCollection);
                        /** 멤버변수 초기화 ******************************/
                        isDataTag = false;
                        mInsertSqlQuery = "";
                        mUpdateSqlQuery = "";
            ...
            GlobalMemberValues.logWrite("coupon_imgtype", "예외발생 : " + e.getMessage());
        }
        ...
        for (String strVec : sqlQueryVec) {
            GlobalMemberValues.logWrite("coupon_imgtype", "----------------------------\n");
            GlobalMemberValues.logWrite("coupon_imgtype", "VECTOR DATA : " + strVec + "\n");
            GlobalMemberValues.logWrite("coupon_imgtype", "----------------------------\n");
        }
        return null;
    }
