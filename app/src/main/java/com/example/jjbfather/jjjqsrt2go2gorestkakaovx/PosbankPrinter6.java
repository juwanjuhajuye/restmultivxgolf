...
public class PosbankPrinter6 {
    ...
    public PosbankPrinter6(Context context, String paramLoc){
        ...
        switch (mLocation) {
            ...
            case "SETTINGS" : {
                dialContext = SettingsDeviceKitchenPrinter5.context;
                break;
            }
            default: {
                if (mContext == null) {
    ...
    public void setConnect(boolean paramDialog){
        if (GlobalMemberValues.PRINTERCONNECTEDSTATUS6 == false) {
            mPrinter = new Printer(mContext, mHandler, null);
            asyncDialog = new ProgressDialog(mContext);
            asyncDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            asyncDialog.setMessage("Loading to connect printer device (POSBANK A11 - kitchen5)...");
            asyncDialog.setCanceledOnTouchOutside(false);
            asyncDialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
                @Override
                public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
            ...
            GlobalMemberValues.logWrite("kitchenprintinglog5", "여기실행...2 " + "\n");
            Toast.makeText(mContext, "PosBank Printer Connect Completed - kitchen5 !", Toast.LENGTH_LONG).show();
            if (paramDialog) {
                if (mContext != null) {
            ...
            switch (msg.what) {
                case Printer.MESSAGE_STATE_CHANGE:
                    switch (msg.arg1) {
                        case Printer.STATE_CONNECTED:
                            ...
                            GlobalMemberValues.PRINTERCONNECTEDSTATUS6 = true;
                            Toast.makeText(mContext, "PosBank Printer Connect Completed - kitchen2 !", Toast.LENGTH_LONG).show();
                            GlobalMemberValues.displayDialog(dialContext, "Printer Connected Message", "PosBank A11 Printer Connect Completed - kitchen5 !", "Close");
                ...
                case Printer.MESSAGE_DEVICE_NAME:
                    ...
                    if (!GlobalMemberValues.isStrEmpty(mConnectedDeviceName)) {
                        ...
                        if (GlobalMemberValues.checkConnectedPosBankKitchPrinterByIp("6",
                        ...
                        String strQuery = " update salon_storestationsettings_deviceprinter6 set " +
                                " printername = '" + SettingsDeviceKitchenPrinter5.selectedItemPrinterNameSpinner + "', " +
                        ...
                        if (returnResult == "N" || returnResult == "") {
                            ...
                            GlobalMemberValues.NETWORKPRINTERIP6 = tempConnectedDeviceName;

                            SettingsDeviceKitchenPrinter5.networkIpTextView1.setText(ipAddrs[0]);
                            SettingsDeviceKitchenPrinter5.networkIpTextView2.setText(ipAddrs[1]);
                            SettingsDeviceKitchenPrinter5.networkIpTextView3.setText(ipAddrs[2]);
                            SettingsDeviceKitchenPrinter5.networkIpTextView4.setText(ipAddrs[3]);

                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName2 : " + ipAddrs[0] + "." + ipAddrs[1] + "." + ipAddrs[2] + "." + ipAddrs[3] + "\n");
                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName6 : " + GlobalMemberValues.NETWORKPRINTERIP6 + "\n");
