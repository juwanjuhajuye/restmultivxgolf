...
public class EpsonLabelPrinter1 implements ReceiveListener {
    ...
    public EpsonLabelPrinter1(Context context){
...
//                mLabel_Printer.addFeedLine(3);
    ...
    private boolean createReceiptData_text(JSONObject data) {
        ...
        try {
            CloverMakingViewInPrinting.makingLabelLinearLayout_text(data, str_printername ,mLabel_Printer);
            mLabel_Printer.addCut(Printer.CUT_FEED);
        } catch (Exception e){
            EpsonPrinterShowMsg.showException(e, method, mContext);
            return false;
...
//    }
    ...
    private void disconnectPrinter() {
        ...
        while (true) {
            try {
                Log.e("jihun epson test", "jihun epson test disconnectPrinter : time - " + sleepTime);
                mLabel_Printer.disconnect();
                break;
            } catch (final Exception e) {
                if (e instanceof Epos2Exception) {
        ...
        MainActivity.mActivity.runOnUiThread(new Runnable() {
            ...
            public synchronized void run() {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Log.e("jihun epson test", "jihun epson test onPtrReceive : time - " + sleepTime);
                        try {
                            Thread.sleep(5000);
