 ...
 */
public class APIDownLoad_salon_storeemployee_worktime extends AsyncTask {

    /** 데이터베이스 테이블 필드값 선언 및 초기화 ********************/
    /** 테이블에 따라 여기를 수정해야 함 ********************/
    String mDbTableName = "salon_storeemployee_worktime";
    ...
    protected Object doInBackground(Object[] params) {
        try {
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (isReturnCode && tagName.equals("ReturnCode")) {
                        ...
                        if (mApiReturnCode.equals("00")) {
                            isPossible = true;
                            GlobalMemberValues.logWrite("salon_storeemployee_worktime", "성공" + "\n");
                        } else {
                            isPossible = false;
                            GlobalMemberValues.logWrite("salon_storeemployee_worktime", "에러" + "\n");
                        }
                        GlobalMemberValues.logWrite("salon_storeemployee_worktime", "ReturnCode : " + mApiReturnCode + "\n");
                    }

                    if (isPossible && isDataTag) {
                        //GlobalMemberValues.logWrite("salon_storeemployee_worktime", "성공일때만.." + "\n");
                        ...
                        if (tagName.equals("sworktime")) {
                            sworktime = xpp.getText();
                        }
                        if (tagName.equals("eworktime")) {
                            eworktime = xpp.getText();
                        }
                        if (tagName.equals("workWeekday")) {
                            workWeekday = xpp.getText();
                        }
                    }
                    ...
                    if (tagName.equals("Data")) {
                        ...
                        mInsertSqlQuery = "insert into " + mDbTableName +
                                " ( " +
                                " idx, employeeidx, sworktime, eworktime, workWeekday" +
                                " ) " +
                                " values (" +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(idx, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(employeeidx, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(sworktime, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(eworktime, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(workWeekday, 0) + "' " +
                                ")";
                        ...
                        mUpdateSqlQuery = "update " + mDbTableName + " set " +
                                ...
                                " sworktime = '" + GlobalMemberValues.getDBTextAfterChecked(sworktime, 0) + "', " +
                                " eworktime = '" + GlobalMemberValues.getDBTextAfterChecked(eworktime, 0) + "', " +
                                " workWeekday = '" + GlobalMemberValues.getDBTextAfterChecked(workWeekday, 0) + "' " +
                                " where idx = " + idx;
                        ...
                        GlobalMemberValues.logWrite("salon_storeemployee_worktime", "mQueryCollection : " + mQueryCollection);
                        /** 멤버변수 초기화 ******************************/
                        isDataTag = false;
                        mInsertSqlQuery = "";
                        mUpdateSqlQuery = "";
            ...
            GlobalMemberValues.logWrite("salon_storeemployee_worktime", "예외발생 : " + e.getMessage());
        }
        ...
        for (String strVec : sqlQueryVec) {
            GlobalMemberValues.logWrite("salon_storeemployee_worktime", "----------------------------\n");
            GlobalMemberValues.logWrite("salon_storeemployee_worktime", "VECTOR DATA : " + strVec + "\n");
            GlobalMemberValues.logWrite("salon_storeemployee_worktime", "----------------------------\n");
        }
        return null;
    }
