...
public class CheckingUploadDataOnSale {
    final String TAG = "CheckingUploadDataOnSaleLog";

    public CheckingUploadDataOnSale() {
        //
    }

    public boolean checkDataInCloud(boolean paramIsService) {
        // 테이블
        String tblName = "salon_sales_detail";
        // 결과값
        ...
        if (GlobalMemberValues.GLOBALNETWORKSTATUS > 0) {
            ...
            while (resultCursor.moveToNext()) {
                GlobalMemberValues.logWrite(TAG, "여기진입....." + "\n");
                ...
                String tempApi_IDN_COD = tempApi_IDG_COD + "|" + GlobalMemberValues.getDBTextAfterChecked(resultCursor.getString(0), 1);
                if (!GlobalMemberValues.isStrEmpty(tempSalesCode)) {
                    ...
                    if (!GlobalMemberValues.isStrEmpty(returnValue)) {
                        String tempSql = "update " + tblName + " set isCloudUpload = 1 where idx = '" + tempIdx + "' ";

                        GlobalMemberValues.logWrite(TAG, "sql : " + tempSql + "\n");
                        apiUpdateQueryVec.addElement(tempSql);
                    }
                    String returnResult = "";
                    // 트랜잭션으로 DB 처리한다.
                    ...
                    if (returnResult == "N" || returnResult == "") {
                        ...
                        if (paramIsService) {
                            ...
                            // 서비스 중지
                            if (GlobalMemberValues.CURRENTACTIVITYOPENEDSERVICE_1 != null && GlobalMemberValues.CURRENTSERVICEINTENT_1 != null)
                                GlobalMemberValues.CURRENTACTIVITYOPENEDSERVICE_1.stopService(GlobalMemberValues.CURRENTSERVICEINTENT_1);
                        } else {
                            mReturnValue = true;
                        }
                    }
    ...
    public String sendCheckingApi(String strParams) {
        ...
        if (GlobalMemberValues.GLOBALNETWORKSTATUS > 0) {
            if (!GlobalMemberValues.isStrEmpty(strParams)) {
                ...
                try {
                    Thread.sleep(GlobalMemberValues.API_UPLOAD_THREAD_TIME + 2000);
