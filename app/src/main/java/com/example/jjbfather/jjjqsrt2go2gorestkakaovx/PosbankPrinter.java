...
public class PosbankPrinter {
    ...
    public PosbankPrinter(Context context, String paramLoc){
        ...
        switch (mLocation) {
            ...
            case "SETTINGS" : {
                dialContext = SettingsDevicePrinter.context;
                break;
            }
            default: {
                if (mContext == null) {
    ...
    public void setConnectForce() {
        ...
        asyncDialog.setMessage("Loading to connect printer device (POSBANK A11 -- RECEIPT1)...");
    ...
    public void setConnect(boolean paramDialog){
        if (GlobalMemberValues.PRINTERCONNECTEDSTATUS == false) {
            mPrinter = new Printer(mContext, mHandler, null);
            asyncDialog = new ProgressDialog(mContext);
            asyncDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            asyncDialog.setMessage("Loading to connect printer device (POSBANK A11 -- RECEIPT1)...");
            ...
            switch (msg.what) {
                ...
                case Printer.MESSAGE_DEVICE_NAME:
                    ...
                    if (!GlobalMemberValues.isStrEmpty(mConnectedDeviceName)) {
                        String tempConnectedDeviceName = GlobalMemberValues.getReplaceText(mConnectedDeviceName, "/", "");
                        String[] ipAddrs = tempConnectedDeviceName.split("\\.");
                        String strQuery = " update salon_storestationsettings_deviceprinter set " +
                                " printername = '" + SettingsDevicePrinter.selectedItemPrinterNameSpinner + "', " +
                        ...
                        if (returnResult == "N" || returnResult == "") {
                            ...
                            GlobalMemberValues.NETWORKPRINTERIP = tempConnectedDeviceName;

                            SettingsDevicePrinter.networkIpTextView1.setText(ipAddrs[0]);
                            SettingsDevicePrinter.networkIpTextView2.setText(ipAddrs[1]);
                            SettingsDevicePrinter.networkIpTextView3.setText(ipAddrs[2]);
                            SettingsDevicePrinter.networkIpTextView4.setText(ipAddrs[3]);

                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName2 : " + ipAddrs[0] + "." + ipAddrs[1] + "." + ipAddrs[2] + "." + ipAddrs[3] + "\n");
                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName3 : " + GlobalMemberValues.NETWORKPRINTERIP + "\n");
