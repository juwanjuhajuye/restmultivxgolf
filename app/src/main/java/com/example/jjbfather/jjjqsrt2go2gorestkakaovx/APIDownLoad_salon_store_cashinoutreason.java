 ...
 */
public class APIDownLoad_salon_store_cashinoutreason extends AsyncTask {

    /** 데이터베이스 테이블 필드값 선언 및 초기화 ********************/
    /** 테이블에 따라 여기를 수정해야 함 ********************/
    String mDbTableName = "salon_store_cashinoutreason";
    ...
    protected Object doInBackground(Object[] params) {
        try {
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (isReturnCode && tagName.equals("ReturnCode")) {
                        ...
                        if (mApiReturnCode.equals("00")) {
                            isPossible = true;
                            GlobalMemberValues.logWrite("salon_store_cashinoutreason", "성공" + "\n");
                        } else {
                            isPossible = false;
                            GlobalMemberValues.logWrite("salon_store_cashinoutreason", "에러" + "\n");
                        }
                        GlobalMemberValues.logWrite("salon_store_cashinoutreason", "ReturnCode : " + mApiReturnCode + "\n");
                    }

                    if (isPossible && isDataTag) {
                        //GlobalMemberValues.logWrite("salon_store_cashinoutreason", "성공일때만.." + "\n");
                    ...
                    if (tagName.equals("Data")) {
                        ...
                        GlobalMemberValues.logWrite("salon_store_cashinoutreason", "mQueryCollection : " + mQueryCollection);
                        /** 멤버변수 초기화 ******************************/
                        isDataTag = false;
                        mInsertSqlQuery = "";
                        mUpdateSqlQuery = "";
            ...
            GlobalMemberValues.logWrite("salon_store_cashinoutreason", "예외발생 : " + e.getMessage());
        }
        ...
        for (String strVec : sqlQueryVec) {
            GlobalMemberValues.logWrite("salon_store_cashinoutreason", "----------------------------\n");
            GlobalMemberValues.logWrite("salon_store_cashinoutreason", "VECTOR DATA : " + strVec + "\n");
            GlobalMemberValues.logWrite("salon_store_cashinoutreason", "----------------------------\n");
        }
        return null;
    }
