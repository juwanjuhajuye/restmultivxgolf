...
public class EpsonPrinterKitchen1 implements ReceiveListener {
    ...
    public EpsonPrinterKitchen1(Context context){
        ...
        if (mPrinter == null){
            try {
                ...
                MainActivity.mActivity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        EpsonPrinterShowMsg.showExceptionConnectTest(e, "Printer", mContext,"Kitchen Printer 1");
                    }
                });
                return false;
            }
    ...
    private boolean connectPrinter() {
        ...
        try {
            mPrinter.connect("TCP:" + GlobalMemberValues.NETWORKPRINTERIP2, Printer.PARAM_DEFAULT);
        }
        catch (Exception e) {
    ...
    public boolean connectTestPrinter() {
        ...
        try {
            ...
            MainActivity.mActivity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    EpsonPrinterShowMsg.showExceptionConnectTest(e, "connect", mContext,"Kitchen Printer 1");
                }
            });
        ...
        try {
            ...
            MainActivity.mActivity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    EpsonPrinterShowMsg.showExceptionConnectTest(e, "beginTransaction", mContext, "Kitchen Printer 1");
                }
            });
        }
