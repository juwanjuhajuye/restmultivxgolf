...
public class PosbankPrinter5 {
    ...
    public PosbankPrinter5(Context context, String paramLoc){
        ...
        switch (mLocation) {
            ...
            case "SETTINGS" : {
                dialContext = SettingsDeviceKitchenPrinter4.context;
                break;
            }
            default: {
                if (mContext == null) {
    ...
    public void setConnect(boolean paramDialog){
        if (GlobalMemberValues.PRINTERCONNECTEDSTATUS5 == false) {
            mPrinter = new Printer(mContext, mHandler, null);
            asyncDialog = new ProgressDialog(mContext);
            asyncDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            asyncDialog.setMessage("Loading to connect printer device (POSBANK A11 - kitchen4)...");
            asyncDialog.setCanceledOnTouchOutside(false);
            asyncDialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
                @Override
                public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
            ...
            GlobalMemberValues.logWrite("kitchenprintinglog4", "여기실행...2 " + "\n");
            ...
            switch (msg.what) {
                case Printer.MESSAGE_STATE_CHANGE:
                    switch (msg.arg1) {
                        case Printer.STATE_CONNECTED:
                            ...
                            GlobalMemberValues.PRINTERCONNECTEDSTATUS5 = true;
                            Toast.makeText(mContext, "PosBank Printer Connect Completed - kitchen4 !", Toast.LENGTH_LONG).show();
                            GlobalMemberValues.displayDialog(dialContext, "Printer Connected Message", "PosBank A11 Printer Connect Completed - kitchen4 !", "Close");
                ...
                case Printer.MESSAGE_DEVICE_NAME:
                    ...
                    if (!GlobalMemberValues.isStrEmpty(mConnectedDeviceName)) {
                        ...
                        if (GlobalMemberValues.checkConnectedPosBankKitchPrinterByIp("5",
                        ...
                        String strQuery = " update salon_storestationsettings_deviceprinter5 set " +
                                " printername = '" + SettingsDeviceKitchenPrinter4.selectedItemPrinterNameSpinner + "', " +
                        ...
                        if (returnResult == "N" || returnResult == "") {
                            ...
                            GlobalMemberValues.NETWORKPRINTERIP5 = tempConnectedDeviceName;

                            SettingsDeviceKitchenPrinter4.networkIpTextView1.setText(ipAddrs[0]);
                            SettingsDeviceKitchenPrinter4.networkIpTextView2.setText(ipAddrs[1]);
                            SettingsDeviceKitchenPrinter4.networkIpTextView3.setText(ipAddrs[2]);
                            SettingsDeviceKitchenPrinter4.networkIpTextView4.setText(ipAddrs[3]);

                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName2 : " + ipAddrs[0] + "." + ipAddrs[1] + "." + ipAddrs[2] + "." + ipAddrs[3] + "\n");
                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName5 : " + GlobalMemberValues.NETWORKPRINTERIP5 + "\n");
