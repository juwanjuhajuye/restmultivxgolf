 ...
 */
public class APIDownLoad_salon_storediscountbutton extends AsyncTask {

    /** 데이터베이스 테이블 필드값 선언 및 초기화 ********************/
    /** 테이블에 따라 여기를 수정해야 함 ********************/
    String mDbTableName = "salon_storediscountbutton";
            ...
            while (eventType != XmlPullParser.END_DOCUMENT){
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (isPossible && isDataTag) {
                        ...
                        if (tagName.equals("discounttype")) {
                            discounttype = xpp.getText();
                        }
                    ...
                    if (tagName.equals("Data")) {
                        ...
                        mInsertSqlQuery = "insert into " + mDbTableName +
                                " ( " +
                                " idx, scode, sidx, stcode, aid, name, discount, discounttype, useyn, delyn, wdate, sortnum " +
                                " ) " +
                                " values (" +
                                ...
                                "'" + GlobalMemberValues.getDBTextAfterChecked(discount, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(discounttype, 0) + "', " +
                        ...
                        mUpdateSqlQuery = "update " + mDbTableName + " set " +
                                ...
                                " discount = '" + GlobalMemberValues.getDBTextAfterChecked(discount, 0) + "', " +
                                " discounttype = '" + GlobalMemberValues.getDBTextAfterChecked(discounttype, 0) + "', " +
