 ...
 */
public class APIDownLoad_salon_storePG extends AsyncTask {

    /** 데이터베이스 테이블 필드값 선언 및 초기화 ********************/
    /** 테이블에 따라 여기를 수정해야 함 ********************/
    String mDbTableName = "salon_storePG";
    ...
    protected Object doInBackground(Object[] params) {
        try {
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (isReturnCode && tagName.equals("ReturnCode")) {
                        ...
                        if (mApiReturnCode.equals("00")) {
                            isPossible = true;
                            GlobalMemberValues.logWrite("salon_storePG", "성공" + "\n");
                        } else {
                            isPossible = false;
                            GlobalMemberValues.logWrite("salon_storePG", "에러" + "\n");
                        }
                        GlobalMemberValues.logWrite("salon_storePG", "ReturnCode : " + mApiReturnCode + "\n");
                    }
                    ...
                    if (isPossible && isDataTag) {
                        ...
                        if (tagName.equals("pgidx")) {
                            pgidx = xpp.getText();
                        }
                        if (tagName.equals("pgname")) {
                            pgname = xpp.getText();
                        }
                        if (tagName.equals("pgmid")) {
                            pgmid = xpp.getText();
                        }
                        if (tagName.equals("pgid")) {
                            pgid = xpp.getText();
                        }
                        if (tagName.equals("pgpwd")) {
                            pgpwd = xpp.getText();
                        }
                        if (tagName.equals("pgipurl")) {
                            pgipurl = xpp.getText();
                        }
                        if (tagName.equals("pgport")) {
                            pgport = xpp.getText();
                        }
                    ...
                    if (tagName.equals("Data")) {
                        ...
                        mInsertSqlQuery = "insert into " + mDbTableName +
                                " ( " +
                                " idx, sidx, stcode, pgidx, pgname, pgmid, pgid, pgpwd, " +
                                " pgipurl, pgport, useyn, delyn, wdate " +
                                " ) " +
                                " values (" +
                                ...
                                "'" + GlobalMemberValues.getDBTextAfterChecked(pgidx, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(pgname, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(pgmid, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(pgid, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(pgpwd, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(pgipurl, 0) + "', " +
                                "'" + GlobalMemberValues.getDBTextAfterChecked(pgport, 0) + "', " +
                        ...
                        mUpdateSqlQuery = "update " + mDbTableName + " set " +
                                ...
                                " pgidx = '" + GlobalMemberValues.getDBTextAfterChecked(pgidx, 0) + "', " +
                                " pgname = '" + GlobalMemberValues.getDBTextAfterChecked(pgname, 0) + "', " +
                                " pgmid = '" + GlobalMemberValues.getDBTextAfterChecked(pgmid, 0) + "', " +
                                " pgid = '" + GlobalMemberValues.getDBTextAfterChecked(pgid, 0) + "', " +
                                " pgpwd = '" + GlobalMemberValues.getDBTextAfterChecked(pgpwd, 0) + "', " +
                                " pgipurl = '" + GlobalMemberValues.getDBTextAfterChecked(pgipurl, 0) + "', " +
                                " pgport = '" + GlobalMemberValues.getDBTextAfterChecked(pgport, 0) + "', " +
                        ...
                        GlobalMemberValues.logWrite("salon_storePG", "mQueryCollection : " + mQueryCollection);
                        /** 멤버변수 초기화 ******************************/
                        isDataTag = false;
                        mInsertSqlQuery = "";
                        mUpdateSqlQuery = "";
            ...
            GlobalMemberValues.logWrite("salon_storePG", "예외발생 : " + e.getMessage());
        }
        ...
        for (String strVec : sqlQueryVec) {
            GlobalMemberValues.logWrite("salon_storePG", "----------------------------\n");
            GlobalMemberValues.logWrite("salon_storePG", "VECTOR DATA : " + strVec + "\n");
            GlobalMemberValues.logWrite("salon_storePG", "----------------------------\n");
        }
        return null;
    }
