...
public class EpsonReceiptPrintMaster implements ReceiveListener {
    ...
    public EpsonReceiptPrintMaster(Context context){
        this.mContext = context;
    }

    public boolean runPrintReceiptSequence(JSONObject jsonObject, final String printType, JSONObject kitchen, String kitchenYN, String kitchenNum) {
//        jsonObject_kitchen = kitchen;
        ...
        if (kitchen != null && kitchenYN.equals("Y")){
            jsonObject_kitchen = new JSONObject();
            for (Iterator<String> iterator = kitchen.keys(); iterator.hasNext(); ) {
                String  key     = iterator.next();
                Object value    = kitchen.opt(key);

                try {
                    jsonObject_kitchen.put(key, value);
                } catch ( JSONException e ) {
                    //TODO process exception
                }
            }
            try {
                jsonObject_kitchen.put("isMaster","Y");
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
...
//                testPrintN();
                ...
                case "master_kitchen" : {
                    if (!print_Master_kitchen(data, kitchenNum)){
                        finalizeObject();
                        return false;
                    }
                    break;
...
//                break;
            ...
            case "master_kitchen" : {
                getPrintingLn = CloverMakingViewInPrinting.makingLinearLayoutForKitchen(data,printType);
                break;
            }
        }
...
//        getPrintingLn = CloverMakingViewInPrinting.makingLinearLayoutForKitchen(data,  "1");
    ...
    private boolean connectPrinter() {
        ...
        try {
            mPrinter.connect("TCP:" + GlobalMemberValues.NETWORKPRINTERIP_MASTER, Printer.PARAM_DEFAULT);
        }
        catch (Exception e) {
...
//        SaleHistory.mActivity.recreate();     SaleHistory.setSearchSalesHistory();
    ...
    public boolean print_Master_kitchen(final JSONObject data, String kitchenNum) {
        return createReceiptData(data,kitchenNum,"","master_kitchen");
    }
