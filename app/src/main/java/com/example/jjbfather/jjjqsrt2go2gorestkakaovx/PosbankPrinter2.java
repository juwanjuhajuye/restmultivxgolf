...
public class PosbankPrinter2 {
    ...
    public PosbankPrinter2(Context context, String paramLoc) {
        ...
        switch (mLocation) {
            ...
            case "SETTINGS" : {
                dialContext = SettingsDeviceKitchenPrinter1.context;
                break;
            }
            default: {
                if (mContext == null) {
    ...
    public void setConnect(boolean paramDialog){
        if (GlobalMemberValues.PRINTERCONNECTEDSTATUS2 == false) {
            mPrinter = new Printer(mContext, mHandler, null);
            asyncDialog = new ProgressDialog(mContext);
            asyncDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            asyncDialog.setMessage("Loading to connect printer device (POSBANK A11 - kitchen1)...");
            ...
            Toast.makeText(mContext, "PosBank Printer Connect Completed - kitchen1 !", Toast.LENGTH_LONG).show();
            if (paramDialog) {
                if (mContext != null) {
            ...
            switch (msg.what) {
                case Printer.MESSAGE_STATE_CHANGE:
                    switch (msg.arg1) {
                        case Printer.STATE_CONNECTED:
                            ...
                            Toast.makeText(mContext, "PosBank Printer Connect Completed - kitchen1 !", Toast.LENGTH_LONG).show();
                            GlobalMemberValues.displayDialog(dialContext, "Printer Connected Message", "PosBank A11 Printer Connect Completed - kitchen1 !", "Close");
                ...
                case Printer.MESSAGE_DEVICE_NAME:
                    ...
                    if (!GlobalMemberValues.isStrEmpty(mConnectedDeviceName)) {
                        ...
                        if (GlobalMemberValues.checkConnectedPosBankKitchPrinterByIp("2",
                                ipAddrs[0], ipAddrs[1], ipAddrs[2], ipAddrs[3])) {
                            GlobalMemberValues.displayDialog(mContext, "Printer Connected Message",
                                    "The printer you have selected is connected to another kitchen printer.\nDuplicate selection may cause problems. Please select another printer.", "Close");
                        }

                        String strQuery = " update salon_storestationsettings_deviceprinter2 set " +
                                " printername = '" + SettingsDeviceKitchenPrinter1.selectedItemPrinterNameSpinner + "', " +
                        ...
                        if (returnResult == "N" || returnResult == "") {
                            ...
                            GlobalMemberValues.NETWORKPRINTERIP2 = tempConnectedDeviceName;

                            SettingsDeviceKitchenPrinter1.networkIpTextView1.setText(ipAddrs[0]);
                            SettingsDeviceKitchenPrinter1.networkIpTextView2.setText(ipAddrs[1]);
                            SettingsDeviceKitchenPrinter1.networkIpTextView3.setText(ipAddrs[2]);
                            SettingsDeviceKitchenPrinter1.networkIpTextView4.setText(ipAddrs[3]);

                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName2 : " + ipAddrs[0] + "." + ipAddrs[1] + "." + ipAddrs[2] + "." + ipAddrs[3] + "\n");
                            GlobalMemberValues.logWrite("mConnectedDeviceName", "mConnectedDeviceName3 : " + GlobalMemberValues.NETWORKPRINTERIP2 + "\n");
