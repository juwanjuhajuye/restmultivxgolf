 ...
 */
public class APIDownLoad_salon_store_deliveryappcompany extends AsyncTask {

    /** 데이터베이스 테이블 필드값 선언 및 초기화 ********************/
    /** 테이블에 따라 여기를 수정해야 함 ********************/
    String mDbTableName = "salon_store_deliveryappcompany";
    ...
    protected Object doInBackground(Object[] params) {
        try {
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (isReturnCode && tagName.equals("ReturnCode")) {
                        ...
                        if (mApiReturnCode.equals("00")) {
                            isPossible = true;
                            GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "성공" + "\n");
                        } else {
                            isPossible = false;
                            GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "에러" + "\n");
                        }
                        GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "ReturnCode : " + mApiReturnCode + "\n");
                    }

                    if (isPossible && isDataTag) {
                        //GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "성공일때만.." + "\n");
                    ...
                    if (tagName.equals("Data")) {
                        ...
                        mUpdateSqlQuery = "update " + mDbTableName + " set " +
                                ...
                                " appname = '" + GlobalMemberValues.getDBTextAfterChecked(appname, 0) + "', " +
                        ...
                        GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "mQueryCollection : " + mQueryCollection);
                        /** 멤버변수 초기화 ******************************/
                        isDataTag = false;
                        mInsertSqlQuery = "";
                        mUpdateSqlQuery = "";
            ...
            GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "예외발생 : " + e.getMessage());
        }
        ...
        for (String strVec : sqlQueryVec) {
            GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "----------------------------\n");
            GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "VECTOR DATA : " + strVec + "\n");
            GlobalMemberValues.logWrite("salon_store_deliveryappcompany", "----------------------------\n");
        }
        return null;
    }
