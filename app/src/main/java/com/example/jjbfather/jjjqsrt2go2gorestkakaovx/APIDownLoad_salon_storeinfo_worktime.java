 ...
 */
public class APIDownLoad_salon_storeinfo_worktime extends AsyncTask {

    /** 데이터베이스 테이블 필드값 선언 및 초기화 ********************/
    /** 테이블에 따라 여기를 수정해야 함 ********************/
    String mDbTableName = "salon_storeinfo_worktime";
    ...
    protected Object doInBackground(Object[] params) {
        try {
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (isReturnCode && tagName.equals("ReturnCode")) {
                        ...
                        if (mApiReturnCode.equals("00")) {
                            isPossible = true;
                            GlobalMemberValues.logWrite("salon_storeinfo_worktime", "성공" + "\n");
                        } else {
                            isPossible = false;
                            GlobalMemberValues.logWrite("salon_storeinfo_worktime", "에러" + "\n");
                        }
                        GlobalMemberValues.logWrite("salon_storeinfo_worktime", "ReturnCode : " + mApiReturnCode + "\n");
                    }

                    if (isPossible && isDataTag) {
                        //GlobalMemberValues.logWrite("salon_storeinfo_worktime", "성공일때만.." + "\n");
                    ...
                    if (tagName.equals("Data")) {
                        ...
                        mInsertSqlQuery = "insert into " + mDbTableName +
                                " ( " +
                                " idx, sidx, sworktime, eworktime, workWeekday, useyn " +
                                " ) " +
                                " values (" +
                        ...
                        GlobalMemberValues.logWrite("salon_storeinfo_worktime", "mQueryCollection : " + mQueryCollection);
            ...
            GlobalMemberValues.logWrite("salon_storeinfo_worktime", "예외발생 : " + e.getMessage());
        }
        ...
        for (String strVec : sqlQueryVec) {
            GlobalMemberValues.logWrite("salon_storeinfo_worktime", "----------------------------\n");
            GlobalMemberValues.logWrite("salon_storeinfo_worktime", "VECTOR DATA : " + strVec + "\n");
            GlobalMemberValues.logWrite("salon_storeinfo_worktime", "----------------------------\n");
        }
        return null;
    }
